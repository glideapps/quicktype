...
export abstract class ConvenienceRenderer extends Renderer {
    ...
    protected setUpNaming(): OrderedSet<Namespace> {
        ...
        this.topLevels.forEach((t, name) => {
            this.nameStoreView.setForTopLevel(name, this.nameForTopLevel(t, name));
        });
    ...
    private nameForTopLevel = (type: Type, name: string): FixedName => {
        ...
        if (maybeNamedType !== undefined) {
            styledName = defined(this._namedTypeNamer).nameStyle(name);
        } else {
        ...
        if (maybeNamedType !== undefined) {
            ...
            this.nameStoreView.set(maybeNamedType, named);
        }
    ...
    private addNamedForNamedType = (type: Type): Name => {
        ...
        const named = this.globalNamespace.add(new SimpleName(type.getProposedNames(), defined(this._namedTypeNamer)));

        this.addDependenciesForNamedType(type, named);

        this.nameStoreView.set(type, named);
        return named;
    };
