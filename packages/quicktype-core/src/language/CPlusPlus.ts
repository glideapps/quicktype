...
const underscoreValue: [string, NamingStyle] = [
  "underscore-case",
  "underscore",
];
...
const pascalUpperAcronymsValue: [string, NamingStyle] = [
  "pascal-case-upper-acronyms",
  "pascal-upper-acronyms",
];
const camelUpperAcronymsValue: [string, NamingStyle] = [
...
export const cPlusPlusOptions = {
  ...
  typeNamingStyle: new EnumOption<NamingStyle>(
    "type-style",
  ...
  memberNamingStyle: new EnumOption<NamingStyle>(
    "member-style",
  ...
  enumeratorNamingStyle: new EnumOption<NamingStyle>(
    "enumerator-style",
...
export class CPlusPlusRenderer extends ConvenienceRenderer {
  ...
  protected generateClassConstraints(
    ...
    this.forEachClassProperty(c, "none", (_name, jsonName, property) => {
      ...
      res.set(jsonName, [
        this.constraintMember(jsonName),
        "(",
        minMax !== undefined && minMax[0] !== undefined
          ? String(minMax[0])
          : this._nulloptType,
        ", ",
        minMax !== undefined && minMax[1] !== undefined
          ? String(minMax[1])
          : this._nulloptType,
        ", ",
  ...
  protected gatherNlohmannNamespaceForwardDecls(): Sourcelike[] {
    return this.gatherSource(() => {
      this.forEachTopLevel(
        "leading-and-interposing",
        (t: Type, className: Name) => this.emitTopLevelHeaders(t, className)
      );
  ...
  protected emitGenerators(): void {
    if (this._options.justTypes) {
      ...
      let nlohmannNamespaceForwardDecls =
  ...
  protected updateIncludes(
    ...
    for (const t of propTypes) {
      ...
      if (t.type instanceof ClassType) {
        ...
        propRecord.kind =
          t.level === 0 ? IncludeKind.Include : IncludeKind.ForwardDeclare;
        if (t.forceInclude) {
